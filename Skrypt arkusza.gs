...
function timeStamp(rows, duration) {
  source.getSheetByName("WZ*").getRange("A3:C3").setValues([[new Date(), rows, duration]]);
}
...
function onOpen() {
  ...
  .addItem('Boczna lista terenów', 'territoryTables')
...
function copyEntriesWZ() {
  ...
  for (var number = firstTerritory; number <= lastTerritory; number++) {
      ...
      territoryDataList.push(territoryData);
      if (number % 25 == 0 ) {
          source.toast(number + " TIME: " + Math.round((Date.now() - start)/1000), "Status", 2);
      };
    //break;
  };
